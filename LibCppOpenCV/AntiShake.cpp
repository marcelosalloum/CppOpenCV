...
cv::Mat AntiShake::getHomographyFeedbackController(Mat &img_1, Mat &img_2,
	...
	do {
		loopCounter++;
		try {
			homography = antiShake(workImage1, workImage2, MATCHES_MEAN_DIST, 60); // exceptions could appear here... //STEPS 1 to 8 there.
...
// IT IS APPLYIED PRIOR TO ANTI-SHAKE
...
// ANTI-SHAKE: IS COMPOSED OF SEVERAL STEPS:
cv::Mat AntiShake::antiShake(Mat &img_1, Mat &img_2, int matches_type, int numberOfMatches) {
	...
	cout << "==== STEP 7 complete: finished matching descriptors: "
	...
	cout << "==== STEP 8 complete: finished calculating right homographY."
			<< endl;
...
// TODO ---------- CROP IMAGE'S BLACK FRAME (private):
struct SortOnX
{
	...
	if(kpts.size() > 0){
		...
		for (unsigned int pVar = 0; pVar < pointsX.size(); ++pVar) {
			...
			for (unsigned int cVar = 0; cVar < corners.size(); ++cVar) {
				if((abs(pointsX[pVar].x - corners[cVar].x) < img.cols/3) && (abs(pointsX[pVar].y - corners[cVar].y) < img.rows/3)){
					isNew = false;
		...
		if(corners.size() != 4){
			...
			cout<< "points[0] (x,y) = (" << points[0].x << ", "<< points[0].y<< ")"<< endl<<
					"points[1] (x,y) = (" << points[1].x << ", "<< points[1].y<< ")"<<
					endl<<"(gray.cols, gray.rows) = ("<< gray.cols << ", "<< gray.rows <<")"<< endl;
			if(points[0].x == 12 && points[0].y == 12 && (gray.cols - points[1].x  == 20) && (gray.rows - points[1].y == 20)){
...
// Shows the image in a window TODO receive only filename
